...
do
	...
	echo sizemesh $nnref
	mkdir -p ./CoarseSnapshots/hdiv2/$tau/$param/
	mkdir -p ./CoarseSnapshotsPhi/hdiv2/$tau/$param/
	FreeFem++-nw Crank_EulerSensitivityinit.edp -tau $tau -nnref $nnref -Param $param -theta $theta
	mv Snapshoth* ./CoarseSnapshots/hdiv2/$tau/$param/
	mv SnapshotPhih* ./CoarseSnapshotsPhi/hdiv2/$tau/$param/
    done
done
...
do
	...
	echo sizemesh $nnref
	mkdir -p ./FineSnapshots/$tau/$param/
	mkdir -p ./FineSnapshotsPhi/$tau/$param/
	FreeFem++-nw Crank_EulerSensitivityinit.edp Crank_Eulerinit.edp -tau $tau -nnref $nnref -Param $param -theta $theta
	mv Snapshoth* ./FineSnapshots/$tau/$param/
	mv SnapshotPhih* ./FineSnapshotsPhi/$tau/$param/
    done
done
